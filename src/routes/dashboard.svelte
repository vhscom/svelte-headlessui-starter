<h2>Welcome to SvelteKit Headless UI Starter</h2>
<p>Launch your SvelteKit app using Tailwind and Headless UI.</p>
<ul>
	<li>
		Visit <a href="https://codeberg.org/vhs/sveltekit-headlessui-starter">codeberg.org</a> to view the
		source code
	</li>
	<li>
		Visit <a href="https://tailwindcss.com">tailwindcss.com</a> to read the Tailwind documentation
	</li>
	<li>
		Visit <a href="https://headlessui.dev/">headlessui.dev</a> to read the Headless UI documentation
	</li>
	<li>
		Visit <a href="https://kit.svelte.dev">kit.svelte.dev</a> to read the SvelteKit documentation
	</li>
	<li>
		Visit <a href="https://svelte.dev">svelte.dev</a> to read the Svelte documentation
	</li>
</ul>
